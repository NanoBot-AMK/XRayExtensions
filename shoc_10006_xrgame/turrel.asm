...
; ecx - CCar_CHolderCustom	// 24Ch
	...
	.else		;// выход
		...
		xorps	xmm0, xmm0
		movflt	xmm0, -1.0	; дистанция выхода
...
; edi - K	CKinematics*
	...
	.if ([edx].tag==ectFirst)
		...
		.if (ecx)
			...
			float_fchs	[ecx].r_torso.yaw, [edx].yaw		; меняем знак
...
;перекрестия прицела машины с оружием CCarWeapon
;вызывается из void CActor::UpdateCL()
align_proc
